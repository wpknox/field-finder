...
- **⚠️ Two `frontend-design` skills now exist** — the new plugin (`frontend-design:frontend-design`) and a pre-existing project-local one at `.claude/skills/frontend-design/` listed in CLAUDE.md's table. Different files, near-identical names. Name the one you mean when invoking.
